...
| IRSA role | `litellm-Cluster-ClusterLitellmSaRoleB6A0F44D-K4BkyERtnoUn` |
...
- AgentCore Gateway 调用日志**默认不开**：Metrics 默认有（`AWS/Bedrock-AgentCore`），
  但请求/响应体 Logs 要手动开 vended log delivery（已开，见 [docs/05 issue #5](docs/05-issues-and-gotchas.md#issue-5)）。
