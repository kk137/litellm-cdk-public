...
> **效果**横向实测。测试从生产环境的 LiteLLM Pod（新加坡 ap-southeast-1）发起，反映真实链路。
> 本报告为内部技术评估，不含密钥；测试日期 2026-06-23。
...
| **agentcore** | 裸 SigV4 调 AgentCore MCP（C2a） | AWS 托管 @ us-east-1 | AWS IAM / IRSA |

> `_execute_search` 内部即 `litellm.asearch(query, search_provider)`，故 exa/tavily/searxng
> 用原生 provider、agentcore 用 C2a 自定义实现，三者在 interception 中角色等价，对比公平。
...
- **AgentCore 的价值不在性能排名**，而在工程与治理维度（见对比表）：
  - 全托管，免维护搜索引擎（对比 searxng 要自己跑容器/维护）；
  - 搜索流量不出 AWS 网络；
  - 复用 IAM/IRSA 鉴权，无需第三方 API Key 与账号；
  - 计费并入 AWS 账单。
- **适用判断**：已重度使用 AWS/Bedrock、看重数据不出网与凭证统一治理的团队，AgentCore 的省心
...
> 附：原始数据见同目录 `bench.latency.csv`（延迟）、`*.results.jsonl`（搜索结果原文）、
> `eval.scores.csv`（逐 query 逐维度评分）、`eval.judge.jsonl`（盲评映射与 judge 理由原文）。
> 测试脚本：`bench_search.py`（延迟）、`judge_search.py`（LLM-judge）。
