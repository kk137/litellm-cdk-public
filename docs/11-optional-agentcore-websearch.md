...
本仓的 litellm config 由 `cdk/lib/helpers/model-config-builder.ts` 在 synth 时生成——**持久改动应改 TS 再 `cdk deploy`**。下面的手改 live ConfigMap 只作**临时验证**用（下次 deploy 会被 TS 源覆盖），基于 **live 真实配置**修改：
...
async def m():
    ...
    rows=await db.query_raw("""
      ...
      where (metadata->>'"'"'user_api_key_team_alias'"'"')='"'"'<TEAM_ALIAS>'"'"'
